...
int main(int argc, char* argv[])
{
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        i=0;
        while(p!=NULL){
            if(i==0) { printf("job_id=%s ", p); job_arr[idx].job_id = atoi(p); }   
            if(i==1) { printf("submit_time=%s ", p); job_arr[idx].submit = 100 + atoi(p); }  // why submit cannot start from 0? 
            if(i==3) { printf("duration=%s ", p); job_arr[idx].duration = atoi(p); }   
            if(i==7) { printf("tasks=%s ", p); job_arr[idx].tasks = ceil((float) (atoi(p)/CPUS_PER_NODE)); }
            //if(i==7) { printf("%s", p); job_arr[idx].tasks = ceil((float) atoi(p)); }
            if(i==8) { printf("wlcimit=%s\n", p); job_arr[idx].wclimit = atoi(p); }   
